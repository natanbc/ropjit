...
int main(void) {
    ...
    data[1] = (uint64_t)smash_the_stack;
    data[2] = (uint64_t)ropjit_gadget_call(ROPJIT_REG_RAX);
    ...
    data[3] = (uint64_t)ropjit_gadget_load_immediate(ROPJIT_REG_RSI);
    data[4] = (uint64_t)data;
    data[5] = (uint64_t)ropjit_gadget_load_immediate(ROPJIT_REG_RDI);
    data[6] = (uint64_t)&real_code;
    ...
    data[7] = (uint64_t)ropjit_gadget_load(ROPJIT_REG_RDI, ROPJIT_REG_RDI);

    //data[0..2] need to be restored, so 3 * uint64_t values have to be copied
    data[8] = (uint64_t)ropjit_gadget_load_immediate(ROPJIT_REG_RCX);
    data[9] =           3 /* 0, 1, 2 */ * sizeof(uint64_t);

    //this could be any register other than RCX, RDI and RSI
    data[10] = (uint64_t)ropjit_gadget_load_immediate(ROPJIT_REG_RAX);
    data[11] = (uint64_t)ropjit_copy_memory;
    //"call it", abusing the ret in ropjit_copy_memory to run the next instruction
    data[12] = (uint64_t)ropjit_gadget_jump(ROPJIT_REG_RAX);

    //load the address again. Since RDI is still valid, it could just be subtracted,
    //so the commented code would also work
    /*
    data[13] = (uint64_t)ropjit_gadget_load_immediate(ROPJIT_REG_RAX);
    data[14] =           3 * sizeof(uint64_t);
    data[15] = (uint64_t)ropjit_gadget_sub(ROPJIT_REG_RDI, ROPJIT_REG_RAX);
    data[16] = (uint64_t)ropjit_gadget_move(ROPJIT_REG_RSP, ROPJIT_REG_RDI);
    */
    data[13] = (uint64_t)ropjit_gadget_load_immediate(ROPJIT_REG_RAX);
    data[14] = (uint64_t)&real_code;
    data[15] = (uint64_t)ropjit_gadget_load(ROPJIT_REG_RAX, ROPJIT_REG_RAX);
    data[16] = (uint64_t)ropjit_gadget_move(ROPJIT_REG_RSP, ROPJIT_REG_RAX);
    ...
    //allocate a stack
    uint64_t* mem = malloc(sizeof(uint64_t) * 1000);
    //use only the last bytes for the "code"
    real_code = &mem[980];
    memcpy(real_code, data, 20 * sizeof(uint64_t));

    printf("original: %p, executed: %p\n", data, real_code);
    printf("before ropjit_run\n");
    uint64_t ignored; //infinite loop, no need for the return address
